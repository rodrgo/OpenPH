inline void twist(int *d_rows_mp, int *d_aux_mp, int *d_low, int *d_arglow, const int m, const int p, float *resRecord, float *timeRecord, int *p_iter, dim3 numBlocks_m, dim3 threadsPerBlock_m){
    ...
    cudaMemcpy(h_simplices_with_dim, d_simplex_dimensions, m*sizeof(int), cudaMemcpyDeviceToHost);

    // Get dimension of complex (maximum initial dimension)
    int complex_dim = 0;
    for (int i = 0; i < m; i++){
        complex_dim = h_simplices_with_dim[i] > complex_dim ? h_simplices_with_dim[i] : complex_dim;
    }
    ...
    for(int dim = complex_dim; dim > 0; dim--){
        ...
        minusone_vector_int<<< numBlocks_m, threadsPerBlock_m >>>((int*)d_simplices_with_dim, m);
        find_simplices_with_dim<<<numBlocks_m, threadsPerBlock_m>>>(d_simplex_dimensions, d_simplices_with_dim, dim, m);
